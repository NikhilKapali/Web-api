$(document).ready(function () {
    ...
    $.getJSON('http://localhost:3000/getTable', function (res) {
        $.each(res, function (index) {
            $('#showTable').append('<tr>' + '<td> ' +
                ...
                '<td><button id="delete" class="btn btn-lg btn-primary" value="' + res[index]._id + '">Delete</button></td>' +
                '<td><a class="btn btn-lg btn-primary" href ="updateTable.html?id=' + res[index]._id + '">Update</a></td>' + '</tr>');
        });
    });
    ...
    $('#showTable').on('click', '#delete', function () {
        ...
        //alert(id);
        $.ajax({
            url: 'http://localhost:3000/delete-table/' + id,
            type: 'DELETE',
            dataType: 'json',
            data: id,
